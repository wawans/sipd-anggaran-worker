...
test('get all sub_giat', async ({ page }) => {
  ...
  page.on('response', async (response) => {
    ...
    matchUrl('**/api/master/sub_giat/list_table', url, base) &&
      (endpoint = '/api/master/giatSub');
  ...
  await page.goto('/master/sub_giat?pageIndex=1&pageSize=100', {
    timeout: 300_000,
