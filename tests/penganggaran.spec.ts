...
test('get skpd', async ({ page }) => {
  ...
  if (response.ok()) {
    ...
    try {
      const xhr = await axios.post('/api/getter/anggaran/skpd', {
        data: body.data,
      });
...
test('get all sub kegiatan from skpd', async ({ page }) => {
  ...
  const res = await axios
    .get('/api/getter/anggaran/skpd?status=1')
    .then((r) => r.data);
  ...
  for (const item of res.data) {
    await test.step(`get sub kegiatan from skpd ${item.nama_skpd}`, async () => {
      ...
      const responsePromise = page.waitForResponse(
        '**/api/renja/sub_bl/list_belanja_by_tahun_daerah_unit',
      );
      ...
      if (response.ok()) {
        ...
        try {
          const xhr = await axios.post('/api/getter/anggaran/belanja/sub', {
            data: body.data,
          });
          await expect(xhr.status).toBe(200);
        } catch (e) {
      ...
      const pageSizeOption = await page.locator('mat-option', {
        hasText: '100',
      });
    ...
    // mark item as done
    try {
      const xhr = await axios.put('/api/getter/anggaran/skpd/' + item.id, {
        status_getter: false,
      });
...
test('get all rinci from each sub kegiatan', async ({ page }) => {
  ...
  const res = await axios
    .get('/api/getter/anggaran/belanja/sub?status=1')
    .then((r) => r.data);
  debug(`total data: ${res.data.length}`);
  ...
  for (const item of res.data) {
    ...
    try {
      const xhr = await axios.put(
        '/api/getter/anggaran/belanja/sub/' + item.id,
        { status_getter: false },
      );
      await expect.soft(xhr.status).toBe(200);
    } catch (e) {
